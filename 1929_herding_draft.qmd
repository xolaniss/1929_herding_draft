...
To further understand the relationship between industry herding and political cycles, we turn our attention to the effect of the performance of specific US presidents on industry herding using the following equation:

$$
\gamma_2^{herd} = \theta_0 + \theta_1APPROVAL + \epsilon_{t}, 
$$ {#eq-approval}

where $\gamma_2^{herd}$ is the industry herding coefficient when it is negative (that is, when herding is present), $APPROVAL$ is the presidential approval rating or the presidential economic approval rating, and $\epsilon_{t}$ is the error term. The expectation is that $\theta_1 < 0$, that is, industry herding is negatively related to approval ratings. This is in line with @pastor2020political who suggests that herding is high when uncertainty in high (which occurs when presidential ratings are low). @eq-approval was estimated with OLS and HAC standard errors.
